...
export class DomElement {
     ...
     */
    get<T extends HTMLElement | HTMLElement[] | undefined = HTMLElement>(
        index?: number
    ): T {
     ...
     */
    attr<T extends string | DomElement = string>(
        attr: string,
        value?: string
    ): T {
     ...
     *
     */
    val<T extends string | boolean | undefined | DomElement = undefined>(
        value?: string
    ): T {
        ...
        return this.each((item: HTMLInputElement) =>
            domfn.val(item, value)
        ) as T;
    }
    ...
    private _getElement(
        ...
        if (single) {
            let item = realContext.querySelector(
                selector as string
            ) as HTMLAnyElement;
            return item ? [item] : [];
        }
    ...
    private _getContext(
        context?: Context
    ): HTMLAnyElement | Document | undefined {
        ...
        return document.querySelector(context as string) as
            | HTMLAnyElement
            | undefined;
    }
}
